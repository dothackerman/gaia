use crate::mock::*;
use crate::pallet::{
	ProposalNoCount, ProposalStatus, ProposalVotes, ProposalYesCount,
...
fn submit_proposal_succeeds_for_active_member() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(
			Event::ProposalSubmitted { proposal_id: 1, organizer: ALICE }.into(),
		);
	});
}
...
fn vote_on_proposal_records_vote() {
	new_test_ext().execute_with(|| {
		let id = submit_default(ALICE);
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(ALICE), id, true));
		...
		System::assert_last_event(
			Event::VoteCast { proposal_id: id, voter: ALICE, approve: true }.into(),
		);
	});
}
...
fn no_vote_increments_no_count() {
	new_test_ext().execute_with(|| {
		let id = submit_default(ALICE);
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(ALICE), id, false));
...
fn tally_approves_with_yes_majority() {
	new_test_ext().execute_with(|| {
		let id = submit_default(ALICE);
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(ALICE), id, true));
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(BOB), id, true));
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(CHARLIE), id, false));
		advance_past_voting();
...
fn tally_rejects_with_no_majority() {
	new_test_ext().execute_with(|| {
		let id = submit_default(ALICE);
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(ALICE), id, false));
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(BOB), id, false));
		assert_ok!(Proposals::vote_on_proposal(RuntimeOrigin::signed(CHARLIE), id, true));
		advance_past_voting();
		assert_ok!(Proposals::tally_proposal(RuntimeOrigin::signed(ALICE), id));
		let proposal = ProposalStorage::<Test>::get(id).unwrap();
...
fn execute_proposal_succeeds_for_approved() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(
			Event::ProposalExecuted { proposal_id: id, organizer: ALICE, amount: 100 }.into(),
		);
	});
}
